...
public class StudenteController
{
    ...
    // Endpoint per aggiungere un esame a uno studente specifico
    @PostMapping("/{id}/esami")
    public ResponseEntity<Esame> creaEsamePerStudente(
            @PathVariable Integer id,
            @Valid @RequestBody Esame esame) {

        Esame nuovoEsame = studenteService.aggiungiEsameAStudente(id, esame);
        return new ResponseEntity<>(nuovoEsame, HttpStatus.CREATED);
    }


    @GetMapping("/{id}/media")
    public Map<String, Object> calcolaMediaVodiStudente(@PathVariable Integer id)
    {
        return studenteService.calcolaMediaVotiStudente(id);
    }
    ...
    @GetMapping("/{id}/esami")
    public List<Esame> getEsameOfStudent(@PathVariable("id") int id)
    {
        List<Esame> elencoEsami = studenteService.findEsamiByStudenteIdInOrdine(id);
        return elencoEsami;
    }
